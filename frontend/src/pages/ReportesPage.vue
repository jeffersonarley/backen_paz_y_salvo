...
async function cargar() {
  ...
  try {
    ...
    rows.value = Object.entries(conteo)
      .map(([estado, cantidad]) => ({ estado, cantidad }))
      .sort((a, b) => b.cantidad - a.cantidad)
    tarjetas[0].cantidad = Array.isArray(data) ? data.length : 0
    tarjetas[1].cantidad = conteo['Finalizado'] || 0
    tarjetas[2].cantidad =
      (conteo['EnProceso'] || 0) +
      (conteo['Pendiente de Firmas'] || 0) +
      (conteo['Aprobado'] || 0) +
      (conteo['Borrador'] || 0)
