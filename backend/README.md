...
├── validations/                   # Cadenas de express-validator por recurso
│   ├── authValidation.js
│   ├── contratoValidation.js
│   ├── firmasValidation.js
...
> Estructura alineada con la guía `estructura_frontend`/`backend prueba` del
> profesor: sin carpeta `src/` envolvente, `controller/` en singular,
> `database.js` en la raíz, y las reglas de `express-validator` que antes
> vivían sueltas dentro de cada archivo de rutas ahora están centralizadas en
> `validations/`, igual que en el proyecto de ejemplo.
...
La jerarquía se valida con el middleware `verificarJerarquia` (`middlewares/authMiddleware.js`):
...
- **Política de contraseñas** (`utils/validarPassword.js`): mínimo 8 caracteres, una
...
- **Validación de entrada** (`express-validator`): en endpoints críticos (login, contrato+bienes,
  usuarios, firmas) con `middlewares/validarCampos.js`.
- **Manejo centralizado de errores** (`middlewares/errorHandler.js` + `utils/AppError.js`):
...
- Refactor a `app.js` (app exportable) + `server.js` (solo conexión y listen).
...
- `morgan` para logging; `swagger.js` con documentación en `/api-docs`.
